...
impl ClickListener {
    ...
    fn run_event_loop(
        ...
        let tap_result = CGEventTap::new(
            ...
            move |_proxy, event_type, event| {
                ...
                let click_event =
                    ClickEvent::new(location.x as i32, location.y as i32, button);
    ...
    /// Receive a click event, blocking until one is available or the listener stops.
    ///
    /// Returns `Some(ClickEvent)` if an event is received, `None` if the listener
    /// has been stopped or the channel is disconnected.
    pub fn recv(&self) -> Option<ClickEvent> {
        // Use a timeout loop to allow checking if we've been stopped
        loop {
            if !self.running.load(Ordering::SeqCst) {
                // Try one more time in case there's a pending event
