...
## Releases
- Publish workflow creates a **draft** release — review notes before publishing.
- `.github/release.yml` auto-groups PRs by label into sections (New Features, Bug Fixes, Refactor, Chores).
- Label PRs correctly (`enhancement`/`feat`, `bug`/`fix`, `refactor`, `chore`/`ci`/`docs`/`dependencies`).
